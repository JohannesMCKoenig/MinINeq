...
%first initialize all the new variables and constraints
D = [];
spmd 
    ...
    D_lab=[];
    for i=start:ende
        D_lab=[D_lab;z*w(i)*w((i+1):length(y)).*(yes(i)-yes((i+1):length(y)))+w(i)*w((i+1):length(y)).*(tes(i)-tes((i+1):length(y)))];
    end
end
